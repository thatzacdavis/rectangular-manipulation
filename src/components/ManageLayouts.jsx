      ...
      <div className="flex flex-col justify-between flex-1 h-full">
        <div>
          ...
          <select
            ...
            className="w-full form-select block px-3 py-1.5 text-base text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded focus:text-gray-700 focus:bg-white focus:border-cyan-500 focus:outline-none"
          ...
          >
            {layouts.map((layout, index) => (
              <option key={`layout-${index}`} value={index}>
                {layout.name}
              </option>
            ))}
          </select>
          <button
            className="mt-4 bg-cyan-500 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded disabled:bg-gray-500 disabled:opacity-25"
